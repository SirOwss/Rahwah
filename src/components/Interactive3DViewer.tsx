...
export const Interactive3DViewer = ({
    ...
    switch (modelType) {
      case "traditional-house":
        return {
          ...
          details: ["المساحة: 200 متر مربع", "الطوابق: طابقين + سطح", "الغرف: 3 غرف نوم + صالة + مطبخ", "المواد: طوب طيني + خشب + حجر", "الطراز: عمارة نجدية تقليدية"]
        };
      case "modern-villa":
        return {
          title: "فيلا عصرية",
          details: ["المساحة: 350 متر مربع", "الطوابق: طابق واحد", "الغرف: 4 غرف نوم + مجلس + مطبخ مفتوح", "المرافق: مسبح + حديقة + موقف سيارات", "الطراز: عمارة معاصرة"]
        };
      default:
        return {
          title: "مبنى معماري",
  ...
  return <div className={`relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-lg overflow-hidden ${className}`}>
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2">
        <Button size="sm" variant="outline" className="w-10 h-10 p-0 bg-black/50 border-white/20 text-white hover:bg-black/70" onClick={handleZoomIn}>
          <ZoomIn className="w-4 h-4" />
        </Button>
        <Button size="sm" variant="outline" className="w-10 h-10 p-0 bg-black/50 border-white/20 text-white hover:bg-black/70" onClick={handleZoomOut}>
          <ZoomOut className="w-4 h-4" />
        </Button>
        <Button size="sm" variant="outline" className="w-10 h-10 p-0 bg-black/50 border-white/20 text-white hover:bg-black/70" onClick={handleReset}>
